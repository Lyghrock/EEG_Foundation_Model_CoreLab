...
# Dataset-specific wrapper for the OpenNeuro EEG download root.
...
DATASET_NAME="${DATASET_NAME:-OpenNeuro}"
INPUT_ROOT="${INPUT_ROOT:-${SHARED_EEG_ROOT}/OpenNeuro}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${SHARED_EEG_ROOT}/statistical_reports}"
WORKERS="${WORKERS:-8}"
RAW_FORMATS="${RAW_FORMATS:-edf,bdf,gdf,vhdr,set,fif,fif.gz,cnt,mff,hea}"
