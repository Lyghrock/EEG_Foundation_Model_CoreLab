...
用法:
  ...
  # 8 核并行下载, 下载后自动预处理压缩
  python down_EEG.py --max-size 1000 --max-workers 8 --preprocess
    ...
    try:
        ...
        proc = subprocess.Popen(
            [sys.executable, "-m", "openneuro", "download",
             "--dataset", ds_id, "--target-dir", str(target_dir)],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--target-fs", type=int, default=250, metavar="HZ",
                   help="预处理降采样目标频率 (默认 250 Hz, 0=不降采样)")
...
def main():
    ...
    if args.max_size:
        print(f"  空间上限: {args.max_size:.0f} GB | 全部估计: {total_est:.0f} GB")
    if args.max_workers > 1:
        print(f"  并行下载: {args.max_workers} 个 worker")
    if args.preprocess:
        print(f"  预处理  : 已启用 (降采样→{args.target_fs} Hz, "
              f"{'删除' if not args.no_remove_original else '保留'}原始文件)")
